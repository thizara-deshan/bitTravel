  ...
  const fetchRevenueData = React.useCallback(async () => {
    try {
      ...
      const [overviewRes, packageRes, monthlyRes] = await Promise.all([
        fetch(`http://localhost:5000/api/admin/revenue/overview${baseParams}`, {
          credentials: "include",
        }),
        fetch(`http://localhost:5000/api/admin/revenue/packages${baseParams}`, {
          credentials: "include",
        }),
        fetch("http://localhost:5000/api/admin/revenue/month", {
          credentials: "include",
        }),
      ]);
